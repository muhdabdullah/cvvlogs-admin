...
function EditJob(props) {
  ...
  useEffect(async () => {
    await jobData(localStorage.getItem("auth_id"), id);
  }, []);
  useEffect(async () => {
  ...
  const getAllStatesData = async () => {
    await axios
      .get(`${process.env.REACT_APP_API_END_POINT}/get_country_list.php`)
      .then((res) => {
        setDataStates(res.data.data);
        axios
          .post(`${process.env.REACT_APP_API_END_POINT}/fetch_state_list.php`, {
            country_id:
          ...
          .then((res) => {
            ...
            axios
              .post(
                `${process.env.REACT_APP_API_END_POINT}/fetch_city_list.php`,
                {
                  country_id:
                    props?.editjobReducer?.editjob?.stored_values
  ...
  const handleState = (e) => {
    ...
    if (e.target.value) {
      axios
        .post(`${process.env.REACT_APP_API_END_POINT}/fetch_city_list.php`, {
          country_id: country,
  ...
  let handleTest = (e) => {
    if (lingo.length == 0 && lingCounter == 0) {
      let preIdList = [];
      let previousData = [...props.editjobReducer.editjob.stored_values.skill];
  ...
  const deleteKeySkills = (e) => {
    if (lingCounter == 0) {
      let tempId = [];
      let temp = [...props.editjobReducer.editjob.stored_values.skill];
          ...
          <div className="col-lg-10 col-md-12 shadow p-5 mt-2 create-job-main-recr">
            <div className="row">
              ...
              <div className="col-md-6">
                ...
                <div className="row">
                  <div className="col-md-12 mt-3">
                    ...
                    lingo.length < 1 ? (
                      lingCounter == 0 ? (
                        props.editjobReducer.editjob.stored_values.skill.map(
                          (lang) => (
                            <button
                      ...
                      lingo.map((lang) => (
                        ...
                        >
                          ...
                          {props.editjobReducer.editjob.skills &&
                          props.editjobReducer.editjob.skills.length > 0
                            ? props.editjobReducer.editjob.skills.filter(
                                (data) => data.id == lang
                              ).length > 0
                              ? props.editjobReducer.editjob.skills.filter(
                                  (data) => data.id == lang
              ...
              <div className="col-12">
                <label className="salary-range-recr">Job Description</label>
                <Editor
                  onTextChange={(e) => {
