...
import { DisableOtpAction } from "../../actions/DisableOtpAction";
...
function DisableOtp(props) {
  ...
  const senOtp = async () => {
    ...
    if (code == "") {
      ...
      await props.DisableOtpAction(auth, code);
    }
  };
  const ResendOtp = async () => {
    await fetch(`${process.env.REACT_APP_API_END_POINT}/web/disable_otp.php`, {
      method: "GET",
      headers: {
...
const mapDispatchToProps = (dispatch) => ({
  DisableOtpAction: (userId, code) => dispatch(DisableOtpAction(userId, code)),
});
