...
const MStripe = (props) => {
  ...
  const onPost = () => {
    ...
    fetch(`${process.env.REACT_APP_API_END_POINT}/web/paid_for.php`, {
      ...
      .then((res) => {
        ...
        window.location = `/ApplicantsVideoCv?id=${sid}`;
  ...
  const onToken = (token, addresses) => {
    ...
    var data = {
      ...
      costumer_id: token
        ? token.card
          ? token.card.id !== null && token.card.id !== undefined
            ? token.card.id
            : null
          : null
        : null,
      payment_token: token
        ? token.id !== null && token.id !== undefined
          ? token.id
          : null
        : null,
    };
    ...
    /// post request
    fetch(
      `${process.env.REACT_APP_API_END_POINT}/web/package_payment_confirmation.php`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
